...
*** int, max, min
      if( valnum(arg)!=0 & min!='none' & max!='none' & int='none' ); int=arg; break; endif
      if( valnum(arg)!=0 & min!='none' & max='none' ); max=arg; break; endif
      if( valnum(arg)!=0 & min='none' ); min=arg; break; endif
...
***** Parameter check *****
  ...
  if( (valnum(min)=0 | valnum(max)=0 | valnum(int)=0) & levs='' )
    say 'cannot determine color levels'
    return
  endif
  if( valnum(min)!=0 & valnum(max)!=0 & valnum(int)!=0 & levs!='' )
    say 'multiple definition of color levels'
    return
  endif
...
*** define color
    ...
    while( i <= max-1 )
      ...
      scol = col.i
      ecol = col.ipp
      snum = 16 + (colnum-1)/(max-1.0)*(i-1)
      enum = 16 + (colnum-1)/(max-1.0)*i
      
      defcol( snum, scol, enum, ecol )
